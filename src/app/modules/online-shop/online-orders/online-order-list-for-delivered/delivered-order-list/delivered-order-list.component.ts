...
@Component({
  selector: 'app-delivered-order-list',
  templateUrl: './delivered-order-list.component.html',
  styleUrls: ['./delivered-order-list.component.scss']
})
export class DeliveredOrderListComponent implements OnInit,OnDestroy {
  ...
  getData(storeId: any) {
    ...
    this.filter["orderDeliveryStatus"] = OrderDeliveryStatusEnum.Delivered;
      ...
      async (response) => {
        ...
        if (response && response.result && response.result.items) {
          ...
          data.forEach(element => {
            ...
            element.orderDeliveryStatus = OrderDeliveryStatusEnum.Delivered;
          });
  ...
  getOrderDeliveryStatusList() {
    this.restService.get(`${environment.urls.Get_Order_DeliveryStatus_For_Dropdown}`).subscribe(
      ...
      this.orderDeliveryStatusList = this.orderDeliveryStatusList.filter(status => status.value === OrderDeliveryStatusEnum.Delivered || status.value === OrderDeliveryStatusEnum.Completed);
  ...
  changeOrderDeliveryStatus(orderDeliveryStatus:any,orderId:any){
      ...
      this.restService.get(`${environment.urls.Change_Delivery_Order_Status}?orderId=${orderId}&orderDeliveryStatus=${orderDeliveryStatus}`).subscribe(
            (response) => {  
              if(response.result){
                this.toastr.success(this.translate.instant('online_Delivered_Order_has_been_moved_from_Delivered_to_Complete_Message'), this.translate.instant("toaster_Heading_Success"), { progressBar: true });
                this.getData(this._customUserStoreService.selectedUserStore);
              }
              else{
                this.toastr.error(this.translate.instant('online_Delivered_Order_Invalid_Request_Message'), this.translate.instant("toaster_Heading_Error"), { progressBar: true });
              }
        },
        (err) => {
          console.log(err);
