...
export class PendingOrderListComponent implements OnInit,OnDestroy {
  ...
  getData(storeId: any) {
    ...
    this.filter["orderDeliveryStatus"] = OrderDeliveryStatusEnum.Pending;
      ...
      async (response) => {
        ...
        if (response && response.result && response.result.items) {
          ...
          data.forEach(element => {
            ...
            element.orderDeliveryStatus = OrderDeliveryStatusEnum.Pending;
          });
  ...
  getOrderDeliveryStatusList() {
    this.restService.get(`${environment.urls.Get_Order_DeliveryStatus_For_Dropdown}`).subscribe(
      ...
      this.orderDeliveryStatusList = this.orderDeliveryStatusList.filter(status => status.value === OrderDeliveryStatusEnum.Pending || status.value === OrderDeliveryStatusEnum.Confirmed);
      },
      (err) => {
        console.log(err);
      }
  ...
  changeOrderDeliveryStatus(orderDeliveryStatus:any,orderId:any){
      ...
      this.restService.get(`${environment.urls.Change_Delivery_Order_Status}?orderId=${orderId}&orderDeliveryStatus=${orderDeliveryStatus}`).subscribe(
            (response) => {  
              if(response.result){
                this.toastr.success(this.translate.instant('online_Pending_Order_has_been_moved_from_Pending_to_Confirmation'), this.translate.instant("toaster_Heading_Success"), { progressBar: true });
                this.getData(this._customUserStoreService.selectedUserStore);
              }
              else{
                this.toastr.error(this.translate.instant('online_Pending_Order_Invalid_Request_Message'), this.translate.instant("toaster_Heading_Error"), { progressBar: true });
              }
        },
        (err) => {
          console.log(err);
