...
@Component({
  selector: 'app-shipped-order-list',
  templateUrl: './shipped-order-list.component.html',
  styleUrls: ['./shipped-order-list.component.scss']
})
export class ShippedOrderListComponent implements OnInit,OnDestroy {
  ...
  getData(storeId: any) {
    ...
    this.filter["orderDeliveryStatus"] = OrderDeliveryStatusEnum.Shipped;
      ...
      async (response) => {
        ...
        if (response && response.result && response.result.items) {
          ...
          data.forEach(element => {
            ...
            element.orderDeliveryStatus = OrderDeliveryStatusEnum.Shipped;
          });
  ...
  getOrderDeliveryStatusList() {
    this.restService.get(`${environment.urls.Get_Order_DeliveryStatus_For_Dropdown}`).subscribe(
      ...
      this.orderDeliveryStatusList = this.orderDeliveryStatusList.filter(status => status.value === OrderDeliveryStatusEnum.Shipped || status.value === OrderDeliveryStatusEnum.Delivered);
      },
      (err) => {
        console.log(err);
      }
  ...
  changeOrderDeliveryStatus(orderDeliveryStatus:any,orderId:any){
      ...
      this.restService.get(`${environment.urls.Change_Delivery_Order_Status}?orderId=${orderId}&orderDeliveryStatus=${orderDeliveryStatus}`).subscribe(
            (response) => {  
              if(response.result){
                this.toastr.success(this.translate.instant('online_Shipped_Order_has_been_moved_from_Shipped_to_Delivered_Message'), this.translate.instant("toaster_Heading_Success"), { progressBar: true });
                this.getData(this._customUserStoreService.selectedUserStore);
              }
              else{
                this.toastr.error(this.translate.instant('online_Shipped_Order_Invalid_Request_Message'), this.translate.instant("toaster_Heading_Error"), { progressBar: true });
              }
        },
        (err) => {
          console.log(err);
