...
@Component({
  selector: 'app-confirmed-order-list',
  templateUrl: './confirmed-order-list.component.html',
  styleUrls: ['./confirmed-order-list.component.scss']
})
export class ConfirmedOrderListComponent implements OnInit,OnDestroy {
  ...
  constructor(
    ...
    getData(storeId: any) {
      ...
      this.filter["orderDeliveryStatus"] = OrderDeliveryStatusEnum.Confirmed;
        ...
        async (response) => {
          ...
          if (response && response.result && response.result.items) {
            ...
            data.forEach(element => {
              ...
              element.orderDeliveryStatus = OrderDeliveryStatusEnum.Confirmed;
            });
    ...
    getOrderDeliveryStatusList() {
      this.restService.get(`${environment.urls.Get_Order_DeliveryStatus_For_Dropdown}`).subscribe(
        ...
        this.orderDeliveryStatusList = this.orderDeliveryStatusList.filter(status => status.value === OrderDeliveryStatusEnum.Confirmed || status.value === OrderDeliveryStatusEnum.Shipped);
        },
        (err) => {
          console.log(err);
        }
    ...
    changeOrderDeliveryStatus(orderDeliveryStatus:any,orderId:any){
        ...
        this.restService.get(`${environment.urls.Change_Delivery_Order_Status}?orderId=${orderId}&orderDeliveryStatus=${orderDeliveryStatus}`).subscribe(
              (response) => {  
                if(response.result){
                  this.toastr.success(this.translate.instant('online_Confirmed_Order_has_been_moved_from_Confirmed_to_Shipped_Message'), this.translate.instant("toaster_Heading_Success"), { progressBar: true });
                  this.getData(this._customUserStoreService.selectedUserStore);
                }
                else{
                  this.toastr.error(this.translate.instant('online_Confirmed_Order_Invalid_Request_Message'), this.translate.instant("toaster_Heading_Error"), { progressBar: true });
                }
          },
          (err) => {
            console.log(err);
